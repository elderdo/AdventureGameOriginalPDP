...
C  SO THERE CANT BE MORE THAN 1000 WORDS.)  THESE UPPER LIMITS ARE:
...
C       HIM TO 22 IF HES CARRYING OBJECT 10, AND OTHERWISE WILL GO TO 14.
C            11      303008  49
C            11      9       50
...
C               3       PIRATE DOESNT GO HERE UNLESS FOLLOWING PLAYER
...
C       REMEMBER IF HES READ THE CLUE IN THE REPOSITORY, AND 3 IS USED TO
...
C  SECTION 6S STUFF.  CTEXT(N) POINTS TO A PLAYER-CLASS MESSAGE.  MTEXT IS FOR
...
C  IF SETUP=2 WE DONT NEED TO DO THIS.  IT'S ONLY NECESSARY IF WE HAVEN'T DONE
...
C  SET UP THE ATLOC AND LINK ARRAYS AS DESCRIBED ABOVE.  WELL USE THE DROP
C  SUBROUTINE, WHICH PREFACES NEW OBJECTS ON THE LISTS.  SINCE WE WANT THINGS
C  IN THE OTHER ORDER, WELL RUN THE LOOP BACKWARDS.  IF THE OBJECT IS IN TWO
...
C  DESCRIBED LAST, WELL DROP THEM FIRST.
...
C  CLEAR THE HINT STUFF.  HINTLC(I) IS HOW LONG HES BEEN AT LOC WITH COND BIT
...
C       1       REACHED HALL OF MISTS, BUT HASNT MET FIRST DWARF
...
C  SIXTH DWARF IS SPECIAL (THE PIRATE).  HE ALWAYS STARTS AT HIS CHESTS
...
C       TURNS   TALLIES HOW MANY COMMANDS HES GIVEN (IGNORES YES/NO)
C       LIMIT   LIFETIME OF LAMP (NOT SET HERE)
C       IWEST   HOW MANY TIMES HES SAID "WEST" INSTEAD OF "W"
C       KNFLOC  0 IF NO KNIFE HERE, LOC IF KNIFE HERE, -1 AFTER CAVEAT
C       DETAIL  HOW OFTEN WEVE SAID "NOT ALLOWED TO GIVE MORE DETAIL"
...
1991    K=100
        WRITE(*,1999) LINUSE,LINSIZ,TRVS,TRVSIZ,TABNDX,TABSIZ,KK
...
C  FINALLY, SINCE WERE CLEARLY SETTING THINGS UP FOR THE FIRST TIME...
...
C  CANT LEAVE CAVE ONCE ITS CLOSING (EXCEPT BY MAIN OFFICE).
...
C  THE DWARFS BLOCKING HIS WAY.  IF COMING FROM PLACE FORBIDDEN TO PIRATE
...
C  FIRST OFF, DONT LET THE DWARVES FOLLOW HIM INTO A PIT OR A WALL.  ACTIVATE
C  THE WHOLE MESS THE FIRST TIME HE GETS AS FAR AS THE HALL OF MISTS (LOC 15).
C  IF NEWLOC IS FORBIDDEN TO PIRATE (IN PARTICULAR, IF ITS BEYOND THE TROLL
C  BRIDGE), BYPASS DWARF STUFF.  THAT WAY PIRATE CANT STEAL RETURN TOLL, AND
C  DWARVES CANT MEET THE BEAR.  ALSO MEANS DWARVES WONT FOLLOW HIM INTO DEAD
C  END IN MAZE, BUT CEST LA VIE.  THEYLL WAIT FOR HIM OUTSIDE THE DEAD END.
...
C  THINGS ARE IN FULL SWING.  MOVE EACH DWARF AT RANDOM, EXCEPT IF HES SEEN US
C  HE STICKS WITH US.  DWARVES NEVER GO TO LOCS <15.  IF WANDERING AT RANDOM,
C  THEY DONT BACK UP UNLESS THERES NO ALTERNATIVE.  IF THEY DONT HAVE TO
C  MOVE, THEY ATTACK.  AND, OF COURSE, DEAD DWARVES DONT DO MUCH OF ANYTHING.
...
C  THE PIRATES SPOTTED HIM.  HE LEAVES HIM ALONE ONCE WEVE FOUND CHEST.
...
C  AN UNSEEN CHEST, LET THE PIRATE BE SPOTTED.
        ...
        K=0
        DO 6020 J=50,MAXTRS
C  PIRATE WONT TAKE PYRAMID FROM PLOVER ROOM OR DARK ROOM (TOO EASY!).
...
C  DONT STEAL CHEST BACK FROM TROLL!
...
C  NOW WE KNOW WHATS HAPPENING.  LETS TELL THE POOR SUCKER ABOUT IT.
...
C  THE PROP TO -1-PROP.  THIS WAY OBJECTS WONT BE DESCRIBED UNTIL THEYVE
C  BEEN PICKED UP AND PUT DOWN SEPARATE FROM THEIR RESPECTIVE PILES.  DONT
...
C  EVERY INPUT, CHECK "FOOBAR" FLAG.  IF ZERO, NOTHINGS GOING ON.  IF POS,
...
C  ANALYSE AN OBJECT WORD.  SEE IF THE THING IS HERE, WHETHER WEVE GOT A VERB
...
C  TOTING IT), SO HES FORCED TO USE THE PLOVER-PASSAGE TO GET IT OUT.  HAVING
C  DROPPED IT, GO BACK AND PRETEND HE WASNT CARRYING IT AFTER ALL.

30200   CALL DROP(EMRALD,LOC)
        GOTO 12

C  TRAVEL 303.  TROLL BRIDGE.  MUST BE DONE ONLY AS SPECIAL MOTION SO THAT
C  DWARVES WONT WANDER ACROSS AND ENCOUNTER THE BEAR.  (THEY WONT FOLLOW THE
C  PLAYER THERE BECAUSE THAT REGION IS FORBIDDEN TO THE PIRATE.)  IF
C  PROP(TROLL)=1, HES CROSSED SINCE PAYING, SO STEP OUT AND BLOCK HIM.
...
C  LOOK.  CANT GIVE MORE DETAIL.  PRETEND IT WASNT DARK (THOUGH IT MAY "NOW"
C  BE DARK) SO HE WONT FALL INTO A PIT WHILE STARING INTO THE GLOOM.
...
C  "YOURE DEAD, JIM."
C
C  IF THE CURRENT LOC IS ZERO, IT MEANS THE CLOWN GOT HIMSELF KILLED.  WELL
...
C  WHICH OFFERS REINCARNATION; IF ACCEPT ED, THIS RESULTS IN MESSAGE 82, 84,
...
C  (THIS KLUGE COULD BE CHANGED ONCE WERE SURE ALL REFERENCES TO BIRD AND CAGE
C  ARE DONE BY KEYWORDS.)  THE LAMP IS A SPECIAL CASE (IT WOULDNT DO TO LEAVE
...
C  OLDLOC IS ZAPPED SO HE CANT JUST "RETREAT".
...
C  OKAY, HES DEAD.  LETS GET ON WITH IT.
...
C  CARRY AN OBJECT.  SPECIAL CASES FOR BIRD AND CAGE (IF BIRD IN CAGE, CANT
...
C  CANT ATTACK BIRD BY THROWING AXE.
...
9170    IF(TOTING(ROD2).AND.OBJ.EQ.ROD.AND..NOT.TOTING(ROD))OBJ=ROD2
        ...
        DO 9171 I=1,5
C  NEEDNT CHECK DFLAG IF AXE IS HERE.
...
C  THISLL TEACH HIM TO THROW THE AXE AT THE BEAR!
9176    SPK=164
        CALL DROP(AXE,LOC)
        FIXED(AXE)=-1
        PROP(AXE)=1
...
C  BLAST.  NO EFFECT UNLESS YOUVE GOT DYNAMITE, WHICH IS A NEAT TRICK!
...
C  LOOK UP WD1 IN SECTION 3 OF VOCAB TO DETERMINE WHICH WORD WEVE GOT.  LAST
...
C  READ.  MAGAZINES IN DWARVISH, MESSAGE WEVE SEEN, AND . . . OYSTER?
...
C  BEFORE RESTARTING (SO CANT SAVE THE WORLD BEFORE TRYING SOMETHING RISKY).
...
C  COME HERE IF HES BEEN LONG ENOUGH AT REQUIRED LOC(S) FOR SOME UNUSED HINT.
...
C  TURNS AFTER THE LAST TREASURE HAS BEEN LOCATED (INCLUDING THE PIRATES
...
C  HIM TO TRY TO GET OUT.  IF HE DOESNT WITHIN CLOCK2 TURNS, WE CLOSE THE
...
C  OIL, SINCE THERE ARE BEANSTALKS WHICH WE DONT WANT TO BE ABLE TO WATER,
C  SINCE THE CODE CANT HANDLE IT.  ALSO, WE CAN HAVE NO KEYS, SINCE THERE IS A
...
C  NUMBERS TO SUPPRESS THE OBJECT DESCRIPTIONS UNTIL HES ACTUALLY MOVED THE
C  OBJECTS.
...
C  TO THE TREASURE ACCESSIBLE ONLY VIA THE HALL OF THE MT. KING.  ALSO, HES
C  BEEN IN GIANT ROOM (TO GET EGGS), SO WE CAN REFER TO IT.  ALSO ALSO, HES
C  GOTTEN THE PEARL, SO WE KNOW THE BIVALVE IS AN OYSTER.  *AND*, THE DWARVES
C  MUST HAVE BEEN ACTIVATED, SINCE WEVE FOUND CHEST.
...
C  ONCE HES PANICKED, AND CLOCK2 HAS RUN OUT, WE COME HERE TO SET UP THE
...
C  HAVE BEEN DESTROYED AND NEEDNT BE CARRIED AWAY FROM ITS OLD "PLACE"),
...
C  OUT, AND 12600 IS IF HES WANDERED OUTSIDE AND THE LAMP IS USED UP, IN WHICH
...
C  OH DEAR, HES DISTURBED THE DWARVES.
...
C  DID HE COME TO WITTS END AS HE SHOULD?
...
C  RETURN TO SCORE COMMAND IF THATS WHERE WE CAME FROM.

        IF(SCORNG)GOTO 8241

C  THAT SHOULD BE GOOD ENOUGH.  LETS TELL HIM ALL ABOUT IT.
...
C  WORD2 (CHARS 6 THRU 10 IN WORD2X), ELSE WORD2 IS SET TO ZERO.
        ...
        DATA MASKS/4000000000d+0,20000000d+0,100000d+0,400,2,0/
     1       ,BLANKS/'     '/
